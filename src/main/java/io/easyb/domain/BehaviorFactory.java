  ...
  private static Behavior getStory(GroovyShellConfiguration gShellConfig, File behaviorFile,
    ...
    if (config != null) {
      if (config.getExtendedStoryClass() != null) {
        return new Story(gShellConfig, createPhrase(behaviorFile, pattern), behaviorFile, config, false);
      } else {
        return new Story(gShellConfig, createPhrase(behaviorFile, pattern), behaviorFile, config, true);
      }
    } else {
      return new Story(gShellConfig, createPhrase(behaviorFile, pattern), behaviorFile, config, true);
    }
