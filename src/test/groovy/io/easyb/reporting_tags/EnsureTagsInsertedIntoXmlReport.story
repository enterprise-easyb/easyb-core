...
scenario "nested scenarios", {
  given "a story file with nested scenarios", {
    storyBehavior = BehaviorFactory.createBehavior(new File("${testSourceDir}/groovy/io/easyb/reporting_tags/InsertReportingTagsInto.story"))
  }
  ...
  when "the reports are written", {
    xmlReportLocation = "${reportsDir}/InsertReportingTagsIntoStory-report.xml"
    new XmlReportWriter(xmlReportLocation).writeReport(new ResultsAmalgamator(storyBehavior))
    txtReportLocation = "${reportsDir}/InsertReportingTagsIntoStory-report.txt"
